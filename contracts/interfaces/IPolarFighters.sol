...
interface IPolarFighters {
     ...
     */
    event Trade(address indexed account, TradeType indexed tradeType, uint256[] tokenId, uint256 fragments, uint256 fee);

    /**
     * @dev Emitted when `borrower` collateralizes Nft to borrow tokens.
     */
    event Borrow(address indexed borrower, bytes32[] orderhashes, uint256[] tokenId, uint256 endBlock, uint256 price, uint256 loanquantum, uint256 interest);

    /**
     * @dev Emitted when `payer` redeems Nft.
     */
    event Redeem(address indexed payer, bytes32[] orderhashes, address[] borrowers, uint256[] tokenIds, uint256[] amounts, uint256[] fees);

    /**
     * @dev Emitted when `payer` purchases Nft.
     */
    event Purchase(address indexed payer, bytes32[] orderhashes, address[] borrowers, uint256[] tokenIds, uint256[] amounts, uint256[] fees);
    ...
    /**
     * @dev The quantity of ERC20 tokens per individual fragmented Nft.
     */
